"""
Random Forest Malware Detection Model

This model uses sklearn's RandomForestClassifier for malware detection.
Optimized for Docker deployment with fast inference and reliable performance.
"""
...
class RandomForestMalwareModel:
    """Random Forest based malware detection model compatible with defender framework"""
    
    def __init__(self, 
                 model_path: str = None,
                 thresh: float = 0.5,
                 name: str = 'Random-Forest-Malware-Detector'):
        self.thresh = thresh
        self.__name__ = name
    ...
    def _load_model_components(self):
        """Load the Random Forest model and preprocessing components"""
        try:
            ...
            # Load feature names
            feature_names_path = os.path.join(model_dir, 'random_forest_features.pkl')
            ...
            if self.model and self.scaler and self.feature_names:
                self.is_loaded = True
                logger.info(f"Successfully loaded Random Forest model components")
            else:
    ...
    def predict(self, bytez: bytes) -> int:
        ...
        try:
            ...
            logger.info(f"Random Forest Prediction: {prediction} (confidence: {prediction_proba[1]:.4f}, threshold: {self.thresh})")
            return prediction
