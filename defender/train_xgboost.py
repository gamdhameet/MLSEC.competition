...
# ⬇️ CHANGED: use XGBoost instead of LightGBM
...
def train_with_challenge(ember_dir: str, ember2017_dir: str, dike_dir: str, challenge_goodware: str,
    ...
    logger.info("=" * 80)
    logger.info("TRAINING WITH CHALLENGE DATASET INCLUDED (XGBoost)")
    logger.info("=" * 80)
    ...
    logger.info(f"Ember 2017 dataset: {len(ember_labels)} samples")
    ...
    # ==============================
    logger.info("\n### Training XGBoost (native booster) ###")
    ...
    y_pred_proba = booster.predict(dtest, iteration_range=(0, best_iteration + 1))
    ...
    booster.save_model(os.path.join(model_dir, "vkota_xgb_model.json"))

    with open(os.path.join(model_dir, 'xgb_scaler.pkl'), 'wb') as f:
        pickle.dump(scaler, f)

    with open(os.path.join(model_dir, 'xgb_features.pkl'), 'wb') as f:
        pickle.dump(list(df.columns), f)

    with open(os.path.join(model_dir, 'xgb_threshold.pkl'), 'wb') as f:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train with challenge dataset included (XGBoost)')
